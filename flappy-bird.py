...
pygame.font.init()
STAT_FONT = pygame.font.SysFont('comcsans', 50)

class Bird :
    IMGS = BIRD_IMGS
    # How much we rotate the bird when we go up or down
    MAX_ROTATION = 25
    # How much we rotate every frame / time we move the bird
    ROT_VEL = 20
    # How long we show bird animation 
    ANIMATION_TIME = 5

    def __init__(self, x, y):
        self.x = x
        self.y = y
        # How much the image is tilted
        self.tilt = 0
        # Keep track when we last jump
        self.tick_count = 0
        self.vel = 0
        self.height = self.y
        self.img_count = 0
